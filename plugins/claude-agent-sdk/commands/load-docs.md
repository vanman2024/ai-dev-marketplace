---
description: Load Claude Agent SDK documentation on-demand with intelligent link extraction and parallel WebFetch
argument-hint: [feature-name|core|all]
allowed-tools: Task, Read, Bash
---

# Load Claude Agent SDK Documentation

**Purpose:** Load fresh Claude Agent SDK documentation on-demand by extracting external links from local docs and fetching them in priority-based batches.

## Command Arguments

**Syntax:** `/claude-agent-sdk:load-docs [argument]`
...
/claude-agent-sdk:load-docs core

# Load all documentation (comprehensive)
/claude-agent-sdk:load-docs all

# Load documentation for specific feature
/claude-agent-sdk:load-docs streaming
/claude-agent-sdk:load-docs tools
/claude-agent-sdk:load-docs providers
```
...
```markdown
Task(
  description="Load Claude Agent SDK documentation",
  subagent_type="doc-loader-agent",
  prompt="Load documentation for Claude Agent SDK with the following parameters:

  **Plugin Information:**
  - Plugin Name: Claude Agent SDK
  - Documentation Path: plugins/claude-agent-sdk/docs/
  - Loading Scope: $SCOPE

  **Instructions:**
  1. Find all markdown documentation files in plugins/claude-agent-sdk/docs/
...
```markdown
## Claude Agent SDK Documentation Loaded
...
- `/claude-agent-sdk:load-docs core` - Essentials only
- `/claude-agent-sdk:load-docs all` - Everything
- `/claude-agent-sdk:load-docs {feature}` - Specific feature

**To build features:**
Use the relevant slash commands:
- `/claude-agent-sdk:new-app` - Initialize new project
- `/claude-agent-sdk:add-{feature}` - Add specific features
```

## Configuration

**Plugin Variables (Replace when deploying):**
- `Claude Agent SDK` → Human-readable plugin name
- `claude-agent-sdk` → Command prefix
- `claude-agent-sdk` → Plugin directory path

**Documentation Path:**
- Default: `plugins/claude-agent-sdk/docs/`
...
```
Error: No markdown files found in plugins/claude-agent-sdk/docs/
...
Refer to files in plugins/claude-agent-sdk/docs/ directly.
```
...
- Plugin-specific build commands (e.g., `/claude-agent-sdk:new-app`)
- Plugin-specific feature commands (e.g., `/claude-agent-sdk:add-{feature}`)
- General documentation tools

**Workflow Pattern:**
1. Load documentation with `/claude-agent-sdk:load-docs {feature}`
2. Review documentation for implementation approach
3. Build feature with `/claude-agent-sdk:add-{feature}`
