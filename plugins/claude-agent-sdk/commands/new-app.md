...
You are tasked with helping the user create a new Claude Agent SDK application. Follow these steps in order:

## Step 1: Fetch Latest Documentation (DO THIS FIRST)

Use Context7 MCP to fetch the latest Claude Agent SDK documentation:

1. Use mcp__context7__resolve-library-id with libraryName: "claude-agent-sdk" OR "@anthropic-ai/claude-agent-sdk"
2. Use mcp__context7__get-library-docs with the resolved library ID and topic: "getting started"
3. Use mcp__context7__get-library-docs with topic: "typescript" for TypeScript setup
4. Use mcp__context7__get-library-docs with topic: "python" for Python setup

**CRITICAL**: Do NOT skip these Context7 calls. The documentation may have changed since your training data. Only after fetching these docs should you proceed to Step 2.
...
2. **Check for Latest Versions**:
   - BEFORE installing, use Context7 or check npm/PyPI to find the latest version
...
1. Check for latest package versions using Context7
...
After all files are created and dependencies are installed, invoke the appropriate verifier agent to validate that the Agent SDK application is properly configured and ready for use:

1. **For TypeScript projects**: Invoke the claude-agent-verifier-ts agent to validate the setup
2. **For Python projects**: Invoke the claude-agent-verifier-py agent to validate the setup
...
- **ALWAYS USE LATEST VERSIONS**: Before installing any packages, check for the latest versions using Context7 or by checking npm/PyPI directly
...
Begin by fetching the latest documentation using Context7, then ask the FIRST requirement question only. Wait for the user's answer before proceeding to the next question.
