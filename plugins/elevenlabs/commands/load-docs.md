---
description: Load ElevenLabs documentation on-demand with intelligent link extraction and parallel WebFetch
...
**Purpose:** Load fresh ElevenLabs documentation on-demand by extracting external links from local docs and fetching them in priority-based batches.

## Command Arguments

**Syntax:** `/elevenlabs:load-docs [argument]`
...
/elevenlabs:load-docs

# Load core documentation explicitly
/elevenlabs:load-docs core

# Load all documentation (comprehensive)
/elevenlabs:load-docs all

# Load documentation for specific feature
/elevenlabs:load-docs streaming
/elevenlabs:load-docs tools
/elevenlabs:load-docs providers
```
...
```markdown
Task(
  description="Load ElevenLabs documentation",
  subagent_type="doc-loader-agent",
  prompt="Load documentation for ElevenLabs with the following parameters:

  **Plugin Information:**
  - Plugin Name: ElevenLabs
  - Documentation Path: plugins/elevenlabs/docs/
  - Loading Scope: $SCOPE

  **Instructions:**
  1. Find all markdown documentation files in plugins/elevenlabs/docs/
...
```markdown
## ElevenLabs Documentation Loaded
...
- `/elevenlabs:load-docs core` - Essentials only
- `/elevenlabs:load-docs all` - Everything
- `/elevenlabs:load-docs {feature}` - Specific feature

**To build features:**
Use the relevant slash commands:
- `/elevenlabs:new-app` - Initialize new project
- `/elevenlabs:add-{feature}` - Add specific features
```

## Configuration

**Plugin Variables (Replace when deploying):**
- `ElevenLabs` → Human-readable plugin name (e.g., "Claude Agent SDK", "Vercel AI SDK")
- `elevenlabs` → Command prefix (e.g., "claude-agent-sdk", "vercel-ai-sdk")
- `elevenlabs` → Plugin directory path (e.g., "claude-agent-sdk", "vercel-ai-sdk")

**Documentation Path:**
- Default: `plugins/elevenlabs/docs/`
...
```
Error: No markdown files found in plugins/elevenlabs/docs/
...
Refer to files in plugins/elevenlabs/docs/ directly.
```
...
- Plugin-specific build commands (e.g., `/elevenlabs:new-app`)
- Plugin-specific feature commands (e.g., `/elevenlabs:add-{feature}`)
- General documentation tools

**Workflow Pattern:**
1. Load documentation with `/elevenlabs:load-docs {feature}`
2. Review documentation for implementation approach
3. Build feature with `/elevenlabs:add-{feature}`
